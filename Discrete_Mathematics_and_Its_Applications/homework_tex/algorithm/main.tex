...
% https://tex.stackexchange.com/a/324915/308105
% \newcommand{\ShowLabel}[1]{\texttt{\meaning#1}}
\newcommand{\ShowLabel}[1]{\typeout{\meaning#1}}
...
% \relax https://latex.org/forum/viewtopic.php?p=69056&sid=e478faaefe977da92d6b83aaa1953637#p69056
\newcommand{\CurTodolistToplevelIndex}{\number\numexpr\value{todolisti}\relax}

\newcommand{\AlgLabel}{_default}
% https://tex.stackexchange.com/a/47372/308105
\newcommand{\Suffix}{_default}
...
\newcommand{\ModProcName}[1]{
    ...
    \renewcommand{\AlgLabel}{\CurTodolistToplevelIndex\Suffix}
    % https://tex.stackexchange.com/a/70153/308105
    \renewcommand{\AlgorithmRef}{\text{ See }\hyperref[alg:\AlgLabel]{the pseudocode}}
}

% https://tex.stackexchange.com/a/638275/308105
% https://tex.stackexchange.com/a/108703/308105
\ExplSyntaxOn
...
\NewDocumentCommand{\listenvironments}{m}
 {
  ...
  \clist_map_inline:nn { #1 }
   {
    % https://ctan.math.illinois.edu/macros/latex/base/lthooks-doc.pdf and 
    % https://tex.stackexchange.com/a/566737/308105 similar to the gdb hook

    % ##1 https://www.alanshawn.com/latex3-tutorial/#handle-macro-expansion-like-a-boss
...
\ExplSyntaxOff
\listenvironments{itemize,enumerate,todolist}

% global \renewcommand https://tex.stackexchange.com/a/51750/308105
\makeatletter
...
\newcommand{\ModProcNameAndLabelSuffix}[2]{
    \renewcommand{\ProcTargetName}{\textit{#1}}
    \renewcommand{\NormalProcTargetName}{#1}
    \renewcommand{\CapProcTargetName}{\capitalisewords{#1}.}
    % \RenewCommandCopy{\Suffix}{#2}
    \renewcommand{\Suffix}{#2}
    \renewcommand{\AlgLabel}{\CurTodolistToplevelIndex\Suffix}
    \renewcommand{\AlgorithmRef}{\text{ See }\hyperref[alg:\AlgLabel]{the pseudocode}}
}
...
\SetKwFor{While}{while (\vspace{4pt}}{)}{\vspace{4pt}}
\SetKwFor{For}{for }{}{}
% this doesn't suit for all cases
% \SetKwIF{If}{ElseIf}{Else}{if}{then\vspace{3pt}}{else if}{else}{\vspace{10pt}endif}
...
\newcommand{\ForRangeBy}[5]{%
    % https://www.overleaf.com/learn/latex/Spacing_in_math_mode
    \For{#1\,\(\coloneqq#2\;\)\KwTo \(#3\;\)\By #4}{ #5}
}
\DontPrintSemicolon
...
\newcommand{\CondensedColon}{\colon\!}
...
\begin{todolist}[itemsep=0pt,parsep=0pt,start=3]
    ...
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
    ...
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        ...
        \Else{
            $n=-n$\\
            use this new $n$ to calculate\\CondensedNewline
        }
        \Return{$c$}
    \end{SpacingAlgorithm}
    ...
    \mywrongitem \CapProcTargetName \AlgorithmRef
    \begin{alphlist}
        \item \begin{SpacingAlgorithm}
            {\xcapitalisewords{\NormalProcTargetName}\capitalisewords{ by a brute force algorithm}}{\AlgLabel}
        ...
        \end{SpacingAlgorithm}
        \item \begin{SpacingAlgorithm}
            {\xcapitalisewords{\NormalProcTargetName}\capitalisewords{ based on sorting}}{\CurTodolistToplevelIndex_b}
    ...
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        ...
        \(end=i\)\\
        % binary insertion 
        % the following comments are from the original binary insertion code
        \While{\(s<end\)}{
            \(c=\lfloor\frac{s+end}{2}\rfloor\)\\
            % \lIf{\(x\le a_c\)}{
            \lIf{\(a_{i+1}\le a_c\)}{
    ...
    \mywrongitem \CapProcTargetName \AlgorithmRef
    % https://tex.stackexchange.com/questions/603954/fully-expand-an-argument-before-using-it-in-latex#comment1514660_603954
    \ShowLabel{\expandafter{\(\AlgLabel\)}}
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        ...
        % correspond to the Ans:"add i to the proposal list for the woman j who ranks highest on his preference list"
        ...
        \While{\(\text{each row of}\{r_{ji}\}\text{ has exactly one }2\hcancel{\SepAnd\text{the rest are all }1}\)}{
            ...
            \ForRange{j}{1}{3}{
                ...
                \tcc{rejection}
                \ForRange{i}{1}{3}{
                    \tcc{Here only the preferred ones can be left pending} \label{only_one_chosen_deferred_acceptance}
                    \If
            ...
            \item At the first round
                  \begin{itemize}
                      \item No proposal overlap: if no men propose to the same woman, then trivially no rejected men.
                            So this is stable. (It \textbf{prefers to the men's side}) \label{prioritize_men}
    ...
    \begin{SpacingAlgorithm}{\xcapitalisewords{\NormalProcTargetName}\capitalisewords{ in wikipedia}}{\CurTodolistToplevelIndex_wiki}
        ...
        % the following comments show the relation with the above pseudocode.
    ...
    \end{enumerate}
    \addtocounter{todolisti}{-1}
    \mywrongitem Here I put the discussion about the above algorithm together with the algorithm in this pdf
    for the better representation instead of in the markdown doc.\\
    See the ans. Here sum up the ans
    ...
    \begin{enumerate}
        ...
        \item Input, Output, Definiteness(ensured by the \hyperref[alg:\number\numexpr\CurTodolistToplevelIndex-1\Suffix]{C-code}), Generality(size \(s\)): trivial
    ...
    \end{alphlist}
    \addtocounter{todolisti}{-1}
    \mywrongitem Here I put the proof together with the algorithm in this pdf instead of in the markdown doc.\\
    ...
    \ecc{
        ...
        \item \href{https://en.wikipedia.org/wiki/Boyer-Moore_majority_vote_algorithm\#Correctness}{wikipedia}
...
\section{section 3 supplementary}
\begin{todolist}[resume*,start=5]
    \ModProcNameAndLabelSuffix{finds the maximum and minimum of a sequence}{-3-supplementary}
    % _ is one special character, which is a bit hard to use in different cmds, so not use it.
    % \ModProcNameAndLabelSuffix{finds the maximum and minimum of a sequence}{_3_supplementary}

    % \item \ifinenumerate{enumerate}{not enumerate}
    ...
    \begin{alphlist}
        ...
        \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
            \myproc{\ProcTargetName(\(\{a_1, a_2,\ldots , a_n\}: \text{a list}\))}{}
            \(max,min\mycmt{max,min\text{ are the maximum and minimum
                    of the sequence correspondingly}}\)\\
            \(LocalMax,LocalMin\)\\
            \Comment{
                \(LocalMax,LocalMin\text{ are the local maximum and minimum
                    of the sequence correspondingly}\)}
            \lIf{n\(\text{ is odd}\)}{
                \(max,min\coloneqq 1\)
            }\ElseIf{\(a_1<a_2\)}{
        ...
        \end{SpacingAlgorithm}
        \item \(2+\frac{[n-(1\text{ or }2)]*3}{2}=O(n),
        \text{with -2 taken when n is even}\)\\
        Here I take ``n is odd" comparison
        in account in the first \(2\),
    \end{alphlist}

    \addtocounter{todolisti}{4}
    \ModProcName{bidirectional bubble sort}
    \myrightitem \CapProcTargetName \AlgorithmRef\\
    similar to 3.1-39.
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        \myproc{\ProcTargetName(\(\{a_1, a_2,\ldots , a_n\}: \text{a list}\))}{}
    ...
    \addtocounter{todolisti}{16}
    \ModProcName{sum triple search}
    \mywrongitem \CapProcTargetName \AlgorithmRef\\
    Hinted by the ans.
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        \myproc{\ProcTargetName(\(\{a_1, a_2,\ldots , a_n\}: \text{a list}\))}{}
        \(find\coloneqq 0\mycmt{find\text{ tracks whether we find the triple}}\)\\
        \ForRange{i}{1}{n-2}{
            \ForRange{j}{i+1}{n-1}{
                \ForRange{k}{j+1}{n}{
                    \If{\(a_k=a_i+a_j\)}{
    ...
    \end{SpacingAlgorithm}
    \ecc{
    \item The above is same as 29... with
    \(
    \sum_{i=1}^{n-2}(\frac{(n-i)(n-i-1)}{2})=O(n^3)
    \)
    }
    \myrightitem See the above \hyperref[prioritize_men]{code description} ("since the code is based on men primarily")
    \begin{enumerate}
        \item \sout{See the \hyperref[only_one_chosen_deferred_acceptance]{code} for why
                  always at least one ``male optimal''}
    \end{enumerate}

    \addtocounter{todolisti}{10}
    \ModProcName{makespan greedy algorithm}
    \myrightitem \CapProcTargetName \AlgorithmRef\\
    Based on 42, this algorithm is not optimal...
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        \myproc{\ProcTargetName(\(\{t_1, t_2,\ldots , t_n\}: \text{the non-decreasing job list},p:\text{processor count}\))}{}
        \(L_k,k\in\{1,2,\ldots,p\}\coloneqq 0\mycmt{L_k\text{ is the processor } k \text{ load}}\)\\
        \(min\coloneqq 1\mycmt{min\text{ is the min processor load index}}\)\\
        \ForRange{i}{1}{n}{
        \ForRange{j}{1}{p}{
            \lIf{\(L_{min}>L_j\)}{
                \(min=j\)
    ...
    \end{SpacingAlgorithm}
    ``in order'' means \(1\colon\!n\)
        instead of \(t_1<t_2<\ldots<t_n\)
\end{todolist}

\section{4.2}
\begin{todolist}[resume*,start=54]
    \ModProcNameAndLabelSuffix{Cantor expansion}{-4-2}
    \mywrongitem \CapProcTargetName \AlgorithmRef
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        \myproc{\ProcTargetName(\(x: \text{number to be transformed}\))}{}
        \(S\coloneqq\varnothing\mycmt{\text{expansion index list}}\)\\
        \(quotient\coloneqq 1\mycmt{quotient\text{ is the division quotient}}\)\\
        \(remainder=x\)\\
        \While{\(remainder\neq 0\)}{
            \(i\coloneqq 1\)\\
            \While{\(quotient\neq 0\)}{
                % https://tex.stackexchange.com/a/128498/308105
                % \(quotient=\ensuremath{\frac{\displaystyle x}{\displaystyle i!}}\)
                \(i=i+1\)\\
    ...
    \end{SpacingAlgorithm}
    \ecc{
        \item better use \(a_{x-1}=\hcancel{1}\;quotient\) and use \(\lfloor\ldots\rfloor\) for \textbf{div}
        \item similar to the ans, better iterate among \(1\CondensedColon x_0-1\)
            after finding the first \(x\) which is \(x_0\).\\
            More specifically, remove the outer while loop.
        \item $\textbf{while }(n+1)\cdot f\le x$ is to decide the $n$ and let $f=n!$.
    }

    % The Same in Binary https://www.cs.cornell.edu/~tomf/notes/cps104/twoscomp.html
    \addtocounter{todolisti}{2}
    \ModProcName{the difference of two binary expansions}
    \mywrongitem \CapProcTargetName \AlgorithmRef
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        \myproc{\ProcTargetName(\(a_{n-1}\ldots a_0,b_{n-1}\ldots b_0: \text{binary expansion of a,b in a-b}\))}{}
        \(c_{n-1}\ldots c_0\mycmt{\text{result}}\)\\
        \(carry,borrow\coloneqq 0\)\\
        \ForRange{i}{0}{n-1}{
            \If{\(a_i<b_i\)}{
                \(c_i=2+a_i-b_i-borrow\)\\
                \(borrow=1\)
            }\Else{
                \(c_i=a_i-b_i-borrow\)\\
                \(borrow=0\)
    ...
    \end{SpacingAlgorithm}
    \ecc{
        \item should be \(a_i<b_i+borrow\)
    }
    \addtocounter{todolisti}{-1}
    \mywrongitem based on 59, \(<4n\Rightarrow O(n)\)
\end{todolist}

\section{4.3}
\begin{todolist}[resume*,start=7]
    % example 4
    \ModProcNameAndLabelSuffix{prime factorization}{-4-3}
    \mywrongitem \CapProcTargetName \AlgorithmRef
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        \myproc{\ProcTargetName(\(x: \text{number to be factorized}\))}{}
        \(dividend\coloneqq 2\)\\
        \(i\coloneqq 1\)\\
        \While{\(x\text{ is not prime}\)}{
            \While{\(dividend\nmid x\)}{
                \(\text{let }dividend\text{ equals the next prime}\)\\
            }
            \vspace{2pt}
            \(a_i=dividend\)\\
            \(x=x/dividend\)\\
    ...
    \end{SpacingAlgorithm}
    \begin{enumerate}
        \item the ans just iterate by step 1 with \(dividend\)
        \item better use \(\bmod\) to indicate "not divide"
        \item the ans stops when \(x=1\) although it is longer.
    \end{enumerate}
\end{todolist}

\section{6.6}
\begin{todolist}[resume*,start=11]
    \ModProcNameAndLabelSuffix{all r-permutations}{-6-6}
    \mywrongitem \CapProcTargetName \AlgorithmRef\\
    This is translated from the ans.
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        \myproc{\ProcTargetName(\(S=\{a_1,a_2,\ldots,a_n\}: \text{a set of n elements}\))}{}
        \(RComb_{s}\coloneqq \{a_1,a_2,\ldots,a_r\}\)\Comment{initial r-comb}
        \(TmpPerm\coloneqq RComb_{s}\)\\
        \(TmpComb\coloneqq RComb_{s}\)\\
        \(P\coloneqq \varnothing\)\\
        \While{\(TmpComb\neq \{a_{n-r+1},...,a_n\}\)}{
            \While{\(TmpPerm\neq inverse(TmpComb)\)}{
                \(\text{append }P\text{ with }TmpPerm\)\\
                \tcc{"generate all the permutations of the elements in this combination"}
                \(TmpPerm=NextPermutation(TmpPerm)\)
            }
            \tcc{"Using Algorithm 3, we generate all the r-combinations of the set with n elements."}
            \(TmpComb=NextRCombination(TmpComb)\)\Comment{This implies the increasing order}
            \(TmpPerm=TmpComb\)
        }
        \Return{\(P\)}
    \end{SpacingAlgorithm}
    ...
    \addtocounter{todolisti}{3}
    \ModProcName{Cantor digits to permutation}
    \mywrongitem \CapProcTargetName \AlgorithmRef
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\AlgLabel}
        \myproc{\ProcTargetName(\(S=\{s_1,s_2,\ldots,s_n\}: \text{a set of n elements},i:\text{integer less than n!} \))}{}
        \(\{a_i\}\)\Comment{Cantor digits}
        \(T\coloneqq \{\overbrace{t_1,\dots,t_n}^{n\text{ times}}=\{\overbrace{1,\dots,1}^{n\text{ times}}\}\)\\
        \(IndexList\coloneqq \varnothing\)\\
        \(index\coloneqq 0\)\\
        \ForRange{k}{1}{n-1}{
            \(a_k=\lfloor\frac{i}{k!}\rfloor\bmod (k+1)\)
        }
        \ForRange{k}{1}{n-1}{
            \tcc{1.break out of the loop if "len(IndexList)" equals 0}
            \ForRange{j}{0}{len(IndexList)}{
                \While(\tcc*[h]{correspond to $h\coloneqq n$}){\(n-a_{n-k}\le IndexList[j]\)}{
                    \(a_{n-k}=a_{n-k}+1\)
                }
                \Else{
                    \Break
            ...
            \(index=n-a_{n-k}\)\\
            \(t_{index}=s_{n-k+1}\)\\
            \(\text{append }index\text{ to }IndexList\text{ and sort it in the increasing order.}\)
        }
        \(index=\{1,\dots,n\}\setminus IndexList\)\\
        \(t_{index}=s_1\)
    \end{SpacingAlgorithm}
    \ecc{
        \item $\textbf{while }p_h\neq 0$ will exclude already filled slots.\\
            Then with $\textbf{for }j\coloneqq 1\textbf{ to }c$, it will implement $(n-i)-a_{n-i-1}$ as exercise 15 shows.\\
            Then the correct location $h$ is decided.
        \item it uses $c\coloneqq \dots$ to calculate $a_k$ at each iteration which \textbf{decreases space usage}.
        \item $x/(n-k)!\Rightarrow k<n$, so we have one ending $h\dots$ to find the location for $s_1$.
        \item The above code has \textbf{extra overheads} of sort based on the ans.\\
            But it condenses many $h\coloneqq h-1$ inside $\textbf{for }j\coloneqq 1\textbf{ to }c$ into one $index=n-a_{n-k}$.
    }
\end{todolist}
