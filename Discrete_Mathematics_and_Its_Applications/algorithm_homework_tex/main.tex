...
\begin{todolist}[itemsep=0pt,parsep=0pt,start=3]
    ...
    \begin{itemize}
        \item Here I prove $i>j$ when out of the \textbf{while} loop is impossible.\\
              ...
              \item The ans shows \(i<j-2\) which excludes the following cases:
                    \begin{CondensedCases}
                        ...
                        \end{cases}
                    \end{CondensedCases}
                    This also implies the above already proved \(i=j\).\\
                    And the above \textbf{correct} ``exclude/allow'' are implied by the above \(>\Rightarrow+1\)
                    ...
                    More specifically, the above excluded cases does the ending assignments shown in the ans:
    ...
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\CurTodolistToplevelIndex}
        ...
        \If{$a_{j+1}>a_j$}{
        $swap=1$\\
        $\displaystyle \text{swap }a_{j+1}\text{ and }a_j$
        }
        }
        \If{$swap=0$}{
            \Break
    ...
    \ecc{
        ...
        \item the above should be \(x\le a_i\) and return \(i\).
    ...
    \ecc{
        \item The above conclusion \ref{item_if_3_1_46} is \textbf{wrong} because at least for the last item \(a_n\),
        ...
        \item For conclusion \ref{item_else_3_1_46}, although for \(a_1\), \(a_0\) doesn't exist. 
