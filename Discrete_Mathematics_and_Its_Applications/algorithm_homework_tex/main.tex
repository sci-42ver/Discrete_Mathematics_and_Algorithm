...
\begin{todolist}[itemsep=0pt,parsep=0pt,start=3]
    ...
    \mywrongitem \CapProcTargetName \AlgorithmRef
    \begin{SpacingAlgorithm}{\CapProcTargetName}{\CurTodolistToplevelIndex}
        \myproc{\ProcTargetName($\{a_1, a_2,\ldots , a_n\}: \text{a sorted increasing list, }x:\text{target}$)}{}
        \ForRange{i}{1}{n}{
            \If{\(x\ge a_i\)}{
                \Break
            }
    ...
    \end{SpacingAlgorithm}
    \ecc{
        \item it should return \(i+1\) when $i\neq n+1$ and 0 when $i=n+1$
        \item it will continue if all $<$
    }
    \addtocounter{todolisti}{-1}
